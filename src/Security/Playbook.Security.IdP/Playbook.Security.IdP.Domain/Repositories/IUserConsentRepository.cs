...
{
    /// <summary>
    /// Checks for an existing active consent for a specific User and Client.
    /// </summary>
    Task<UserConsent?> GetActiveConsentAsync(UserId userId, string clientId, CancellationToken ct = default);

    /// <summary>
    /// Retrieves all active consents for a user (used for the 'Authorized Apps' UI).
    /// </summary>
    Task<IEnumerable<UserConsent>> GetActiveConsentsByUserIdAsync(UserId userId, CancellationToken ct = default);
}
